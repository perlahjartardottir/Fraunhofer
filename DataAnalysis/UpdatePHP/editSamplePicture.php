...
$temp = explode(".", $_FILES["sample_picture_new"]["name"]);
...
if(isset($_POST["submit"])) {
    $check = getimagesize($_FILES["sample_picture_new"]["tmp_name"]);
    if($check !== false) {
...
// }
// Check file size
if ($_FILES["sample_picture_new"]["size"] > 500000) {
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["sample_picture_new"]["tmp_name"], $target_file)) {
        // echo "The file ". basename( $_FILES["sample_picture_new"]["name"]). " has been uploaded.";
    } else {
